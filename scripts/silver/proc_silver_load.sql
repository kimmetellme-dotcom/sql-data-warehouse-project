...
BEGIN
	...
	BEGIN TRY
		...
		SELECT 
			...
			prd_nm,
			ISNULL(prd_cost, 0) AS prd_cost, --IF NULL,replace with the new value
			CASE UPPER(TRIM(prd_line))
				...
				WHEN 'T' THEN 'Touring'
				ELSE 'n/a'
			END prd_line, --Map product line code to descriptive value
			prd_start_dt,
			CAST(
				LEAD(CAST(prd_start_dt AS DATETIME)) OVER(PARTITION BY prd_key ORDER BY CAST(prd_start_dt AS DATETIME))-1 
				AS DATE
